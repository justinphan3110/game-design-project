...
public class ArcherTower : MonoBehaviour, Tower
{
    ...
    void Start () {
        ...
        status = PREPARING;
        peek_tower = transform.TransformPoint(new Vector3(-1.476951f, 6.93142f, 0.9330738f));
        floor_tower = peek_tower;
        floor_tower.y = 0f;

        arrow_obj = (GameObject)Instantiate(Resources.Load("Arrow"));
    ...
    void shooting()
    {
        try
        {
            ((Character)capture_enemy).take_hp(Time.deltaTime * damage / shoot_time, true);
        }
        catch
        {}
        ...
        if ((time_shoot_pass += Time.deltaTime) < shoot_time)
        {
            ...
            target_position.y = peek_tower.y * Mathf.Cos(Mathf.PI / 2 / shoot_time * time_shoot_pass);
