...
public class Footman : MonoBehaviour, Character, Hero
{
    const double EPSILON = 0.1;

    [HideInInspector] public Blacksmith blacksmith;
    [HideInInspector] public Vector3 prepare_point;
    ...
    void Update()
    {
        ...
        if (blacksmith.gameObject.activeInHierarchy == false)
            DestroyImmediate(gameObject);
        else
        switch (status)
        {
            case HeroStatus.DEATH:
                animator.Play("Footman_Death");
            ...
            case HeroStatus.EXTRA_ATTACKING:
                if (finding_enemy(HeroStatus.RANGE_EXTRA_ATTACK, EnemyStatus.RUNNING))
                {
                    ...
                    enemy_walker.being_captured(GetComponent<Footman>());
    ...
    bool finding_enemy(int hero_range, int enemy_status)
    {
        ...
        foreach (Component capture_enemy in blacksmith.figures_script.enemy_walkers)
        {
    ...
    bool Character.take_hp(float damage_other, bool is_physical_other)
    {
        ...
        if (!is_living)
        {
            if (blacksmith.gameObject.activeInHierarchy == true)
                blacksmith.add_hero(this);
