...
public class OrcWarrior : MonoBehaviour, Character, EnemyWalker
{
    ...
    void Start()
    {
        ...
        figures_script.enemy_walkers.Add(GetComponent<OrcWarrior>());
    ...
    void Update()
    {
        ...
        switch (status)
        {
            ...
            case EnemyStatus.DEATH:
                animator.Play("OrcWarrior_Death");
                //Destroy(gameObject, GetComponent<Animation>().clip.length);
                Destroy(gameObject, Figures.TIME_DEATH_NEED);
                break;
    ...
    void attacking()
    {
        try
        {
            animator.Play("OrcWarrior_Attack");
